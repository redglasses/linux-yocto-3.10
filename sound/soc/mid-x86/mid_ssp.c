		...
		struct snd_soc_dai *cpu_dai)
{
	...
	if (!cpu_dai->active) {
		if (!strcmp(cpudai_drv->name, SSP0_PCM_DAI_NAME)) {
			ssp_config->i2s_handle =
				intel_mid_i2s_open(SSP_USAGE_PCM_CARD_0);
			pr_debug("opening the CPU_DAI for "\
					"SSP_USAGE_PCM_CARD, i2s_handle = %p\n",
					ssp_config->i2s_handle);
		}
		else if (!strcmp(cpudai_drv->name, SSP1_PCM_DAI_NAME)) {
			ssp_config->i2s_handle =
				intel_mid_i2s_open(SSP_USAGE_PCM_CARD_1);
			pr_debug("opening the CPU_DAI for"\
					"SSP_USAGE_PCM_CARD, i2s_handle = %p\n",
					ssp_config->i2s_handle);
		}
		else if (!strcmp(cpudai_drv->name, SSP2_PCM_DAI_NAME)) {
			ssp_config->i2s_handle =
				intel_mid_i2s_open(SSP_USAGE_PCM_CARD_2);
			pr_debug("opening the CPU_DAI for"\
					"SSP_USAGE_PCM_CARD, i2s_handle = %p\n",
					ssp_config->i2s_handle);
		}
		...
		int clk_id, unsigned int freq, int dir)
{
	...
	if ((freq < cpu_dai->driver->playback.rate_min) || (freq > cpu_dai->driver->playback.rate_max)) {
		pr_err("SSP DAI: %s Bad freq_out=%d (beyond range of %dHz - %dHz \n",
				__func__,
				freq,
				cpu_dai->driver->playback.rate_min,
				cpu_dai->driver->playback.rate_max);
		return -EINVAL;
	}
	...
	switch (freq) {
		...
		case 22050:
			...
			break;
		case 18900:
			i2s_config->master_mode_standard_freq = SSP_FRM_FREQ_18_900;
			break;
		case 16000:
...
struct snd_soc_dai_driver intel_ssp_platform_dai_sample[] = {
	{
		.name = SSP0_PCM_DAI_NAME,
		.playback = {
...
static struct snd_soc_dai_driver intel_ssp_platform_dai[] = {
	{
		.name = SSP0_PCM_DAI_NAME,
		.playback = {
			.channels_min = SSP_STEREO_CHANNEL,
			.channels_max = SSP_TDM8_CHANNEL,
			.rates = SSP_SAMPLE_RATES,
			.rate_min = SSP_MIN_RATE,
