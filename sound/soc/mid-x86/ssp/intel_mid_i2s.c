...
static inline void i2s_enable(struct intel_mid_i2s_hdl *drv_data, const struct intel_mid_i2s_settings *ps_settings, int streamdir)
{
		...
		if(GET_SSSR_val(sssr, ROR)) {
			...
			if(STEREO == ps_settings->frame_rate_divider_control) { /*I2S fix*/
				if (SSPSCLK_SLAVE_MODE == ps_settings->sspslclk_direction)
					while((read_SSSR(drv_data->ioaddr) & SSSR_CSS));	/*Wait CSS equal 0 before re-enable SSP*/
	...
	/*Enable TX/RX for requested stream operation*/
	if(STEREO == ps_settings->frame_rate_divider_control) {
...
static void ssp1_dump_registers(struct intel_mid_i2s_hdl *drv_data)
{
	...
	{
		dev_dbg(ddbg, "LPE_SSP0_DIVC_L: 0x%08x\n", read_LPE_SSP0_DIVC_L(reg));
		dev_dbg(ddbg, "LPE_SSP0_DIVC_H: 0x%08x\n", read_LPE_SSP0_DIVC_H(reg));
	}
	else if(SSP1_INSTANCE == drv_data->device_instance)
	{
		dev_dbg(ddbg, "LPE_SSP1_DIVC_L: 0x%08x\n", read_LPE_SSP1_DIVC_L(reg));
		dev_dbg(ddbg, "LPE_SSP1_DIVC_H: 0x%08x\n", read_LPE_SSP1_DIVC_H(reg));
	}
	else if(SSP2_INSTANCE == drv_data->device_instance)
	{
		dev_dbg(ddbg, "LPE_SSP2_DIVC_L: 0x%08x\n", read_LPE_SSP2_DIVC_L(reg));
		dev_dbg(ddbg, "LPE_SSP2_DIVC_H: 0x%08x\n", read_LPE_SSP2_DIVC_H(reg));
	}
...
static int get_frequency(enum mrst_ssp_frm_freq freq){
	int requested_freq;
	switch (freq) {
		...
		u8 *frame_rate_divider_new)
{
	...
	/* frequency */
	requested_freq = get_frequency(freq);
	if(0xFFFF == requested_freq) {
		...
		const struct intel_mid_i2s_settings *ps_settings)
{
	...
	u32 required_bandwidth = 0;
	u32 div_m = 0;
	u32 div_n = 0;
	u32 clk_common_div = 0;
	int freq;
	int ssp_slot_width;
	...
	if (l_ssp_clk_frm_mode == SSP_IN_MASTER_MODE) {
		...
		freq = get_frequency(ps_settings->master_mode_standard_freq);
		if(0xFFFF == freq)
		{
			dev_err(ddbg,"Unsupported frequency in SSP as master mode\n");
			goto leave;
		}

		/* Limit SSP slot width to validated sizes of 8, 16 and 32 bits.
		 * Developers may change to the desired slot width at own risk*/
		if (ps_settings->data_size <= 8)
			ssp_slot_width = 8;
		else if (ps_settings->data_size <= 16)
			ssp_slot_width = 16;
		else if (ps_settings->data_size <= 32)
			ssp_slot_width = 32;
		else {
			ssp_slot_width = 32;
			dev_WARN(ddbg,"SSP unsupported slot width : %d, forced to %d\n"
					, ps_settings->data_size, ssp_slot_width);
		}

		required_bandwidth = ssp_slot_width * frame_rate_divider * freq;

		if(required_bandwidth > CLOCK_SOURCE_KHZ) {
			dev_err(ddbg, "Requested clock %d Hz exceeded clock source range "
					"of %d Hz\n", required_bandwidth, CLOCK_SOURCE_KHZ);
			goto leave;
		}

		clk_common_div = gcd(CLOCK_SOURCE_KHZ, required_bandwidth);
		div_m = required_bandwidth/clk_common_div;
		if(MN_MAX < div_m) {
			dev_err(ddbg, "m value of 0x%08X too large! (max = 0xFFFFF)\n"
					, div_m);
			goto leave;
		}

		div_n = CLOCK_SOURCE_KHZ/clk_common_div;
		if(MN_MAX < div_n) {
			dev_err(ddbg, "n value of 0x%08X too large! (max = 0xFFFFF)\n"
					, div_n);
			goto leave;
		}

		dev_dbg(ddbg, "M/N divisor values: m = %d, n = %d\n", div_m, div_n);
	...
	ssp_divc_h |= LPE_SSP0_DIVC_H_reg(DIV_BYPASS, 0) /*0=Disable Bypass bit*/
		|LPE_SSP0_DIVC_H_reg(DIV_EN, 1) /*1=Enable the m/n divider*/
		|LPE_SSP0_DIVC_H_reg(DIV_UPDATE, 1) /*1=Enable the update bit*/
		|LPE_SSP0_DIVC_H_reg(DIV_M, div_m);
	ssp_divc_l |= LPE_SSP0_DIVC_L_reg(DIV_N, div_n);
	...
	*usage = SSP_USAGE_UNASSIGNED;
	switch ( (drv_data->paddr & 0xFFFFF) ) {
		case BYT_SSP0_START_ADDRESS:
			*usage	= SSP_USAGE_PCM_CARD_0;
			pr_debug("%s : Matched usage to card 0 \n", __func__);
			break;
		case BYT_SSP1_START_ADDRESS:
			*usage	= SSP_USAGE_PCM_CARD_1;
			pr_debug("%s : Matched usage to card 1 \n", __func__);
			break;
		case BYT_SSP2_START_ADDRESS:
			*usage	= SSP_USAGE_PCM_CARD_2;
			pr_debug("%s : Matched usage to card 2 \n", __func__);
			break;
		default:
