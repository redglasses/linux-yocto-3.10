/*
 *  byt_ia_sample.c - ASoc Machine driver for Intel Baytrail platform (IA)
 *
		...
		struct snd_pcm_hw_params *params)
{
	 ...
	 * Note: REMOVE/COMMENT OUT the use of snd_soc_dai_set_fmt(codec_dai, fmt)
	 * if use with dummy codec
	 */
		...
		struct snd_pcm_hw_params *params)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_dai *cpu_dai = rtd->cpu_dai;
	/* Add the following lines to if your codec callbacks is needed
	 * struct snd_soc_dai *codec_dai = rtd->codec_dai;
	 */
	...
	/*Set format for CPU DAI*/
	fmt |= SSP_DAI_SCMODE_0; /*Not for use with codec*/
	ret = snd_soc_dai_set_fmt(cpu_dai, fmt);
...
static unsigned int rate_16000[] = {
	16000,
};

static unsigned int rate_18900[] = {
	18900,
};
...
static struct snd_pcm_hw_constraint_list constraints_rate_8000 = {
	.count	= ARRAY_SIZE(rate_8000),
	.list	= rate_8000,
};

static struct snd_pcm_hw_constraint_list constraints_rate_16000 = {
	.count	= ARRAY_SIZE(rate_16000),
	.list	= rate_16000,
};

static struct snd_pcm_hw_constraint_list constraints_rate_18900 = {
	.count	= ARRAY_SIZE(rate_18900),
	.list	= rate_18900,
};

static struct snd_pcm_hw_constraint_list constraints_rate_48000 = {
	.count	= ARRAY_SIZE(rate_48000),
	.list	= rate_48000,
};
...
static int byt_ssp0_ia_dai_link_startup(struct snd_pcm_substream *substream)
{
	...
	if (ret) {
		pr_err("BYT : %s : Fail to set requested channel constraint\n", __func__);
		return ret;
	}

	return snd_pcm_hw_constraint_list(str_runtime, 0,
			SNDRV_PCM_HW_PARAM_RATE,
			&constraints_rate_48000);
}/*byt_ssp0_ia_dai_link_startup*/

/*SSP1: 18.9kHz*/
static int byt_ssp1_ia_dai_link_startup(struct snd_pcm_substream *substream)
{
	int ret = 0;
	struct snd_pcm_runtime *str_runtime;
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	...
	return snd_pcm_hw_constraint_list(str_runtime, 0,
			SNDRV_PCM_HW_PARAM_RATE,
			&constraints_rate_16000);
}/*byt_ssp1_ia_dai_link_startup*/

/*SSP2: 18.9kHz*/
static int byt_ssp2_ia_dai_link_startup(struct snd_pcm_substream *substream)
{
	int ret = 0;
	struct snd_pcm_runtime *str_runtime;
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
...
}/*byt_ssp2_ia_dai_link_startup*/

/*SOC ops for IA path*/
static struct snd_soc_ops byt_ssp0_dai_link_ops = {
	.startup = byt_ssp0_ia_dai_link_startup,
...
static struct snd_soc_ops byt_ssp1_dai_link_ops = {
	.startup = byt_ssp1_ia_dai_link_startup,
...
static struct snd_soc_ops byt_ssp2_dai_link_ops = {
	.startup = byt_ssp2_ia_dai_link_startup,
...
static struct snd_soc_dai_link byt_msic_ssp0_dailink[] = {
	{
		...
		.cpu_dai_name = SSP0_PCM_DAI_NAME,
...
static struct snd_soc_dai_link byt_msic_ssp2_dailink[] = {
	{
		...
		.cpu_dai_name = SSP2_PCM_DAI_NAME,
		.codec_dai_name = "snd-soc-dummy-dai",
		.codec_name = "snd-soc-dummy",
...
{
	pr_info("BYT: Baytrail Machine Driver byt_ia_sample is registering.\n");
...
MODULE_ALIAS("platform:byt-ia-sample");
