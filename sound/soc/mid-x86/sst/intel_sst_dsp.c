...
#ifdef CONFIG_X86_64
	void *src, *dst, *src_index, *dst_index;
	u32 t_size;
#endif
...
#else	/* for 64 bit OS */
		/* byte copy for other architechture */
		src = (void *)block + sizeof(*block);
		dst = ram + block->ram_offset;
		t_size = block->size;
		src_index = src;
		dst_index = dst;
		while (t_size >= 4) {
			memcpy_toio(dst_index, src_index, 4);
			dst_index += 4;
			src_index += 4;
			t_size -= 4;
		}
		if (t_size >= 2) {
			memcpy_toio(dst_index, src_index, 2);
			dst_index += 2;
			src_index += 2;
			t_size -= 2;
		}
		if (t_size == 1)
			memcpy_toio(dst_index, src_index, 1);
#endif
