		...
		const struct pci_device_id *pci_id)
{
	...
	/*1MB DDR cache memory allocation required by BYT*/
	if(sst_drv_ctx->pci_id == SST_BYT_PCI_ID) {
		/*Mem allocation info is in mailbox after this BYT rev*/
		if (pdev->revision > 0x0C) {
			base = readl(sst_drv_ctx->mailbox);
			alloc_len = readl(sst_drv_ctx->mailbox+0x04);
		}
		/* Prior BYT revisions read from PCI CONFIG for allocation info*/
		else {
